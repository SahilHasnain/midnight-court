...
export default function ImageSearchModal({
  ...
  // Auto-fill search query when prefilledKeyword is provided
  useEffect(() => {
    if (visible && prefilledKeyword) {
      setSearchQuery(prefilledKeyword);
      // Auto-trigger search with prefilled keyword
      setTimeout(() => {
        handleSearch(prefilledKeyword);
      }, 300);
    }
  }, [visible, prefilledKeyword]);
  ...
  const handleSearch = async (queryOverride) => {
    ...
    setLoading(true);
    try {
      const results = await searchImages(query.trim(), selectedSource);
      setSearchResults(results);

      if (results.length === 0) {
        Alert.alert("No Results", `No images found for "${query}"`);
      }
    } catch (error) {
      ...
      <View style={styles.container}>
        ...
        {!isTyping && searchResults.length > 0 ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              Browse free stock photos from Unsplash or Pexels. Perfect for
              legal presentations.
            </Text>
          </View>
        )}
